...
- **Super Admin console** — companies list with server-side search, sort and paging (a paged table
  from `lg` up, cards with "Load more" below) and a create-company form that also creates the
  first company admin
- **Workspace shell** — sidebar or top-bar layout on desktop, top bar plus bottom tabs on phones,
  ⌘K screen search, a vendor quick-jump under the company name (opens that vendor's orders),
  notifications panel, account sheet, dark mode, logout
- **Orders** — list with start / return / cancel; New order is a right-hand drawer on the same
  page (`/orders?new=1`, which the phone's "+ New" button opens): vendor, one line per service
  with its options and quantity, and a display-only estimate (the server prices it)
- **Bills** — list with status filter; record a payment (into an account, optional) or void an
  unpaid bill with a reason; download the bill as a PDF (generated by the API on request) or send
  it on WhatsApp — phones attach the PDF through the share sheet, desktops download it and open the
  vendor's chat
- **Settings** — company name, GST number, number prefix, GST and electricity rates
- **Vendor report** — `/vendors/:id` (tap a vendor's name): every order, bill and payment in a date
  range as one timeline, with what they owed going in, after each line, and now
- **Vendors, Service types, Bank, Expenses** — lists with create and edit
- **Daily log** — today's machine hours and electricity units, with an estimated electricity cost;
  a date-ranged history with edit and CSV export
- **PWA** — installable, app shell precached, self-hosted fonts

**Not built:** notifications data, a bill detail page, error monitoring and
product analytics.
...
src/
  ...
  api/
    process-backend/
      ...
      list-query.ts       listQueries (page + infinite factories), usePagedList, fetchAllPages
...
main.tsx
  ...
  │         │     └─ orders, bills, vendors, service-types, bank,
  │         │        expenses, daily-log, settings ─ their pages        (lazy)
...
| `/orders`, `/bills`, `/vendors`, `/service-types`, `/settings` | Company Admin | Orders, bills, vendors, service types, settings |
| `/daily-log`                                                   | Company Admin | Daily log                                       |
| `/vendors/:id`                                                 | Company Admin | Vendor report: orders, bills, payments, dues    |
...
instead of rendering `undefined`. `common.types.ts` holds `ListParams` (`page`, `pageSize`, `q`,
`sort`) and `Paged<T>` (`{ items, total, page, pageSize }`) — the contract every list endpoint
takes; `list-query.ts` turns it into a page query (desktop table) and an infinite query (phone
cards). See [`api-layer.md`](docs/conventions/api-layer.md#paged-lists).
...
Still a placeholder: the notifications panel (`TODO(api)`).
...
| **Companies**                                                              | Super Admin: server-paged list with search, sort, columns and CSV export; create a company plus its first admin, with duplicate-name / phone / email conflicts surfaced per field | Built against the API                    |
| **Workspace shell**                                                        | Sidebar or top-bar desktop layouts, phone top bar + bottom tabs + account sheet, ⌘K palette, notifications, theme toggle, logout                                                  | Built; notifications are a placeholder   |
| Orders, Bills, Vendors, Service types, Bank, Expenses, Daily log, Settings | See the built list above                                                                                                                                                          | Built against the API                    |
