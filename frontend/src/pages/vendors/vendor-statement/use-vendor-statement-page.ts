...
import {
  ...
  type VendorStatementEntry,
  useVendorStatement,
} from '@api/process-backend/vendors';
import { vendorStatementColumns } from '@components/sections/vendors/vendor-statement-table';
...
export interface UseVendorStatementPageResult {
  from: string;
  to: string;
  today: string;
  statement: VendorStatement | undefined;
...
export const useVendorStatementPage = (): UseVendorStatementPageResult => {
  // state
  ...
  const { data, isPending, isError, isFetching, refetch } = useVendorStatement(id, range);
  const showPages = useMediaQuery('(min-width: 64rem)');
  const { exportError, runExport } = useCsvExport('vendor-statement');

  // derived
  // ponytail: the API returns the whole period, so paging is local, as on the bank statement.
  ...
  return {
    ...
    exportCsv: () => runExport(async () => exportRows(vendorStatementColumns, all)),
    retry: () => void refetch(),
  };
};
