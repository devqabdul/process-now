...
| `vendors`       | `/vendors`         | List with search, create, update; per-vendor statement                                                  | create-vendor, update-vendor, list-vendors-query                      | prisma                 |
...
| `daily-logs`    | `/daily-logs`      | List by date range; upsert by date                                                                      | upsert-daily-log, date-range-query                                    | prisma                 |
| `bank-accounts` | `/bank-accounts`   | Accounts (bank or cash) with a balance derived from payments and expenses; per-account statement        | create-bank-account, update-bank-account, date-range-query            | prisma                 |
| `expenses`      | `/expenses`        | Record, edit, delete expenses paid from an account; list by date range; category suggestions            | create-expense, update-expense, list-expenses-query                   | prisma                 |
...
| GET / POST  | `/vendors`                     | List (`q`: name, phone, address; `sort`: `name`, `createdAt`), create                                                                                                                                                  |
| GET         | `/vendors/:id`                 | One vendor                                                                                                                                                                                                             |
| PATCH       | `/vendors/:id`                 | Edit, or set `isActive: false` to retire it                                                                                                                                                                            |
| GET         | `/vendors/:id/statement`       | `from`, `to` → orders received, bills issued and payments made, newest first, each with the amount owed after it; opening and closing due                                                                              |
...
| GET         | `/orders`                      | Filter by `status[]`, `vendorId`; `q` matches vendor, notes, order number; `sort`: `receivedAt`, `orderNo`, `vendor`                                                                                                   |
...
| GET         | `/bills`                       | Filter `status[]` of `due\|paid\|voided` (OR'd); voided hidden otherwise; `q` matches vendor, bill number                                                                                                              |
| GET         | `/bills/:id`                   | With items and payments                                                                                                                                                                                                |
| GET         | `/bills/:id/pdf`               | The bill as a PDF, drawn on request with pdfkit and never stored: `{ fileName, contentType, base64, vendor: { name, phone } }`; amounts read "Rs." because the built-in PDF fonts have no ₹                            |
...
| POST        | `/bills/:id/payments`          | Amount, method (`cash`, `upi`, `bank`, `cheque`, `other`), optional `bankAccountId` (an open account). Rejects an amount above what's due.                                                                             |
...
| GET         | `/dashboard`                   | `date` (defaults to today, IST) → `pendingOrders`, `amountToCollect`, `itemsProcessed[]` (per unit), `machineHours`, `electricityUnits`, `electricityCost`, `earnings`, `estimatedCost`, `estimatedProfit`, `expenses` |
| GET         | `/bank-accounts`               | Each with `openingBalance`, `moneyIn`, `moneyOut`, `balance`                                                                                                                                                           |
| POST        | `/bank-accounts`               | `name` (unique per company), optional `openingBalance`                                                                                                                                                                 |
| GET         | `/bank-accounts/:id`           | One account with its balance                                                                                                                                                                                           |
| PATCH       | `/bank-accounts/:id`           | `name`, `openingBalance`, `isActive` (false closes it to new money; history kept)                                                                                                                                      |
| GET         | `/bank-accounts/:id/statement` | `from`, `to` → payments in and expenses out, newest first, with period totals                                                                                                                                          |
| GET         | `/expenses`                    | `from`, `to`, `bankAccountId`, `category`, `q`, `sort` → paged, plus `sum` (money over all matching rows)                                                                                                              |
| GET         | `/expenses/categories`         | Categories the company has used, for suggestions                                                                                                                                                                       |
| POST        | `/expenses`                    | `bankAccountId`, `category`, `amount`, `spentOn` (not in the future), `notes`                                                                                                                                          |
| PATCH       | `/expenses/:id`                | Any of the above                                                                                                                                                                                                       |
| DELETE      | `/expenses/:id`                | Removes an expense recorded in error                                                                                                                                                                                   |
...
- List endpoints (vendors, service types, orders, bills, expenses, admin companies) share one list layer
  (`src/common/dto/list-query.dto.ts`): `page` (default 1), `pageSize` (15, 25, 50 or 100; default 15), `q`
  (search across the endpoint's text columns) and `sort` (`key`, or `-key` for descending, from a per-endpoint
  allowlist; `id` is always the tiebreaker). They return `{ items, total, page, pageSize }`, `total` counting
  every matching row. Array filters repeat the param (`status=due&status=paid`).
